...
function ChartTooltipContent({
  ...
  labelFormatter?: TooltipContentProps<ValueType, NameType>["labelFormatter"];
  labelClassName?: string;
  formatter?: TooltipContentProps<ValueType, NameType>["formatter"];
  color?: string;
  hideLabel?: boolean;
    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          ...
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
          const rowKey = `${item.dataKey || item.name || key}-${index}`;
          const payloadFill =
  ...
  return (
    ...
    >
      {payload.map((item, index) => {
        ...
        const itemConfig = getPayloadConfigFromPayload(config, item, key);
        const itemKey = `${item.dataKey || item.value || key}-${index}`;
